...
def make_slit(xi,yi,xf,yf,width):
    ...
    if width!=1:
        ...
        # xi=xi_neg+(xi_pos-xi_neg)/(width-1)*np.arange(0,width,0.5,dtype=int)
        # yi=yi_neg+(yi_pos-yi_neg)/(width-1)*np.arange(0,width,0.5,dtype=int)
        # xi=np.asarray(xi,dtype=int)
        # yi=np.asarray(yi,dtype=int)
        xi=xi_neg+(xi_pos-xi_neg)/(width-1)*np.arange(0,width,0.5)
        yi=yi_neg+(yi_pos-yi_neg)/(width-1)*np.arange(0,width,0.5)
        xi=np.asarray(xi)
        yi=np.asarray(yi)
        ...
        # xf=xf_neg+(xf_pos-xf_neg)/(width-1)*np.arange(0,width,0.5,dtype=int)
        # yf=yf_neg+(yf_pos-yf_neg)/(width-1)*np.arange(0,width,0.5,dtype=int)
        # xf=np.asarray(xf,dtype=int)
        # yf=np.asarray(yf,dtype=int)
        xf=xf_neg+(xf_pos-xf_neg)/(width-1)*np.arange(0,width,0.5)
        yf=yf_neg+(yf_pos-yf_neg)/(width-1)*np.arange(0,width,0.5)
        xf=np.asarray(xf)
        yf=np.asarray(yf)
